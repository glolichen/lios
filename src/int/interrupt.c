...
void exception_set_handler(u8 exception, void (*handler)(const struct InterruptData *)) {
...
void handle_exception(const struct InterruptData *data) {
	...
	void (*handler)(const struct InterruptData *) = irq_routines[data->interrupt_num];
	if (handler)
		handler(data);
	asm("cli; hlt");
	while (1);
...
void handle_interrupt(const struct InterruptData *data) {
	...
	void (*routine)(const struct InterruptData *) = irq_routines[irq_number];
	if (routine)
		routine(data);
